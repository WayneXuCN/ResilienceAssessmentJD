...
class MEE(DecisionMethod):
    ...
    def preprocess_data(self):
        ...
        unique_values = {column: self.filled_df[column].unique() for column in self.filled_df.columns}
        # n_values = [0.5, 1, 1.5, 2]
        attribute_0 = self.get_keys_by_value(self.params['criteria_dict'], 'attribute', '0')
        attribute_1 = self.get_keys_by_value(self.params['criteria_dict'], 'attribute', '1')
        attribute_2 = self.get_keys_by_value(self.params['criteria_dict'], 'attribute', '2')
        level_boundaries = {}
        for column, values in unique_values.items():
            """
            # If the indicator is a 0-1 variable, use fixed level boundaries
            if set(values).issubset({0, 1}):
                ...
                # For other indicators, use the mean and standard deviation to divide the grades
                mean = self.filled_df[column].mean()
                std_dev = self.filled_df[column].std()
                level_boundaries[column] = {
                    "待整改": (mean - n_values[3] * std_dev, mean - n_values[2] * std_dev),
                    "合格": (mean - n_values[2] * std_dev, mean - n_values[1] * std_dev),
                    "良好": (mean - n_values[1] * std_dev, mean + n_values[0] * std_dev),
                    "优秀": (mean + n_values[0] * std_dev, mean + n_values[3] * std_dev)
                }
            """
            # For other indicators, use the 25th, 50th, and 75th percentiles to divide the grades
            max_val, min_val = self.filled_df[column].max(), self.filled_df[column].min()
            interval = (max_val - min_val) / 4
            round_digits = 2
            if column in attribute_0:
                level_boundaries[column] = {
                    "待整改": (
                        round(min_val - 0.5 * interval, round_digits), round(min_val + 1 * interval, round_digits)),
                    "合格": (round(min_val + 1 * interval, round_digits), round(min_val + 2 * interval, round_digits)),
                    "良好": (round(min_val + 2 * interval, round_digits), round(min_val + 3 * interval, round_digits)),
                    "优秀": (round(max_val - 1 * interval, round_digits), round(max_val + 0.5 * interval, round_digits))
                }
            elif column in attribute_1:
                level_boundaries[column] = {
                    "待整改": (
                        round(max_val - 1 * interval, round_digits), round(max_val + 0.5 * interval, round_digits)),
                    "合格": (round(max_val - 2 * interval, round_digits), round(max_val - 1 * interval, round_digits)),
                    "良好": (round(max_val - 3 * interval, round_digits), round(max_val - 2 * interval, round_digits)),
                    "优秀": (
                        round(min_val - 0.5 * interval, round_digits), round(min_val + 1 * interval, round_digits))
                }
    ...
    def perform_computation(self, level_boundaries):
        """
        ...
        try:
            correlation_degrees = {}
            for column_name, column_data in self.filled_df.items():
                # 读取经典域
                boundary = level_boundaries.get(column_name)
                # 读取节域
                all_values = [item for sublist in boundary.values() for item in sublist]  # 元素提取出来并扁平化到一个列表中
                ...
                for i in boundary.keys():
                    ...
                    # 利用向量化运算计算关联度
                    _corr.append(pd.Series(
                        np.where(is_in_range,
                                 -distance_cla / np.abs(upper - lower),
                                 np.where(equal_distances,
                                          0,  # 当距离相等时，关联度设为0
                                          distance_cla / (distance_ext - distance_cla))),
                        name=i, index=is_in_range.index))
    ...
    def execute(self):
        ...
        """
        try:
            level_boundaries = self.params.get('level_boundaries') or self.preprocess_data()
            correlation_degrees = self.perform_computation(level_boundaries)
            ...
            corr_comp = {f'{d}_{e}': pd.DataFrame() for d in ['D1', 'D2', 'D3'] for e in ['E1', 'E2', 'E3']}
            corr_comp.update({f'E{i}': pd.DataFrame() for i in range(1, 4)})

            result = []
            for criterion, data in correlation_degrees.items():
                corr_criterion = data[['待整改', '合格', '良好', '优秀']] * weights[criterion].values.reshape(-1, 1)
                    ...
                    if criterion in d_criteria:
                        for e, e_criteria in elements.items():
                            if criterion in e_criteria:
                                key = f'{d}_{e}'
                                corr_comp[key] = self.add_to_df(corr_comp[key], corr_criterion)
                for e, e_criteria in elements.items():
                    if criterion in e_criteria:
