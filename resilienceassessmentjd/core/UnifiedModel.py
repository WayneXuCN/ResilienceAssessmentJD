...
class UnifiedModel:
    ...
    def execute(self):
        ...
        try:
            ...
            if weights['status'] == 'success':
                self.params['weights'] = weights['weights']  # Add the weights to the parameters
            else:
            ...
            if normalized_data['status'] == 'success':
                self.params['norm_data'] = normalized_data['data']  # Add the normalized data to the parameters

            method_instance = DecisionMethodFactory.get_method(self.request.get("assess_method"), self.params)  # Get the method instance
    ...
    def determine_weight(self):
        ...
        if subjective_method and objective_method and combined_method:
            ...
            subj_weights = DecisionMethodFactory.get_method(subjective_method, self.params).execute()
            obj_weights = DecisionMethodFactory.get_method(objective_method, self.params).execute()
            combined_params = {'subjective_weights': subj_weights, 'objective_weights': obj_weights}
            self.params['combined_params'] = combined_params
            combined_instance = DecisionMethodFactory.get_method(combined_method, self.params)
        ...
        # If only the subjective method is provided, return the subjective weights
            ...
            return DecisionMethodFactory.get_method(subjective_method, self.params).execute()
        # If only the objective method is provided, return the objective weights
        elif objective_method:
            return DecisionMethodFactory.get_method(objective_method, self.params).execute()
    ...
    def scaling_data(self):
        ...
        if scaling_method:
            return ScalingMethodFactory.get_method(scaling_method, self.params).execute()
        else:
